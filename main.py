...
# Parâmetros
audio_duration = 7; audio_rate = 5000          # Duração e taxa de amostragem do áudio
...
np.savetxt('./results/input_' +  current_id + '.txt' , x_train.reshape(audio_duration * audio_rate, ), fmt = "%f")
...
def save_intermediate_output(epoch, output):
    np.savetxt('./results/intermediate_output_' +  current_id + '_' + str(epoch) + '.txt', output.numpy().reshape(audio_duration * audio_rate, ), fmt = "%f")
...
np.savetxt('./results/output_' +  current_id + '.txt', best_output[0].numpy().reshape(audio_duration * audio_rate, ), fmt = "%f")

# Salvando os metadados em um arquivo txt
def save_metadata():
    with open('./results/results_metada.txt', 'w') as file:
